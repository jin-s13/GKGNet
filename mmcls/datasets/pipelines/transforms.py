...
class Trivial(object):
    def __init__(self,p):
        self.p=p
        self.aug=TrivialAugment()
    def __call__(self, results):
        if random.random()<self.p:
            results['img']=Image.fromarray(np.uint8(results['img']))
            results['img']=self.aug(results['img'])
            results['img'] = np.asarray(np.uint8(results['img']))
        return results
    def __repr__(self):
...
class CropMixup(object):
    def __init__(self,
        ...
        if isinstance(size, (tuple, list)):
            ...
            self.size = (size, size)
        self.p=p
        self.aug=CropMix(size,scale, mix_ratio, number, operation, inter_aug)
    def __call__(self, results):
        results['img'] = Image.fromarray(np.uint8(results['img']))
        if random.random() < self.p:
            results['img'] = self.aug(results['img'])
            results['img'] = np.transpose(np.uint8(results['img'].numpy() * 255),(1,2,0))
        else:
            pil_image = results['img'].resize(self.size, 3)
            results['img'] = np.array(pil_image)
