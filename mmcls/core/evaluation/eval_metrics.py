...
def precision_recall_f1_subacc(pred, target, average_mode='macro', thrs=0., cal_pos=False):
    ...
    if cal_pos: pred_label = (pred_label >= np.ones(pred_label.shape)).astype(np.int8)
    pred_score = np.sort(pred, axis=1)[:, -1]

    precisions = []
    recalls = []
    f1_scores = []
    t_ms, f_ms, t_fs, f_fs = [], [], [], []  # male=0, female=1
    for thr in thrs:
        ...
        f1_score = 2 * precision * recall / np.maximum(precision + recall,
                                                       1e-20)
        # m:0 f:1 binary
        # print(_pred_label.shape, target.shape)
        t_f = (_pred_label & target).sum()
        f_f = (_pred_label & (1 - target)).sum()
        f_m = ((1 - _pred_label) & target).sum()
        t_m = ((1 - _pred_label) & (1 - target)).sum()
        # false_postive = (pred_positive & gt_negative).sum(0)
    ...
    if return_single:
        return precisions[0], recalls[0], f1_scores[0], t_ms[0], f_ms[0], t_fs[0], f_fs[0]
    else:
        return precisions, recalls, f1_scores, t_ms, f_ms, t_fs, f_fs
